...
def personal_event_report(request, event_id):
    event = PersonalEvent.objects.get(id=event_id)
    from_date = request.GET.get('from_date')
    to_date = request.GET.get('to_date')
    format = request.GET.get('format')
    contributions = event.contributions.filter(status='Paid', created_at__gte=from_date, created_at__lte=to_date)

    # Create a DataFrame and export it to the specified format
    df = pd.DataFrame({
        'Names': [contribution.names for contribution in contributions],
        'Email': [contribution.email for contribution in contributions],
        'Phone': [contribution.phone for contribution in contributions],
        'Amount': [contribution.amount for contribution in contributions],
        'Reference': [contribution.reference for contribution in contributions],
        'Date': [contribution.created_at.strftime('%Y-%m-%d') for contribution in contributions]
    })
